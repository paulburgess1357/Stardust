return {
  {
    ...
    local cfg=require('stardust.config').resolve({stars=0,objects={}})
    ...
    for _,fps in ipairs({1,2,10,30,60,120}) do
      ...
      assert(sky.meteor(scene,view,cfg)); scene.next_meteor=math.huge
  ...
  },
  {
    ...
    reset(lines,{stars=0,meteors=true})
    ...
    assert(sd.meteor()); scene.next_meteor=math.huge
    local body=scene.meteor
    local crossed=false
    for _=1,360 do
  ...
  },
  {
    ...
    [[
    local sky=require('stardust.sky')
    local cfg=require('stardust.config').resolve({stars=0,meteors=false})
    local colors=require('stardust.palette').setup(cfg)
    local view=require('stardust.layout').empty(100,20)
    for _,kind in ipairs({'moon','planet'}) do
      local scene=sky.new(19)
      assert(sky.object(scene,view,cfg,kind)); scene.next_object=math.huge
  ...
  },
  {
    ...
    [[
    local sky=require('stardust.sky')
    local cfg=require('stardust.config').resolve({stars=0,meteors=false,objects={'ship'},ships={{right={' A ','B C'},left={' a ','b c'},color='#abcdef'}}})
    local colors=require('stardust.palette').setup(cfg)
    local view=require('stardust.layout').empty(100,20)
    for _,direction in ipairs({0.25,0.75}) do
      local scene=sky.new(19); scene.random=function() return direction end
      assert(sky.object(scene,view,cfg,'ship')); scene.next_object=math.huge
      local object=scene.object
      sky.step(scene,view,cfg,3,#colors.stars)
      local cells=sky.frame(scene,view,cfg,colors)
      assert(#cells == 3)
      local chars={}
      for _,cell in ipairs(cells) do chars[cell.glyph]=true; assert(cell.hl == colors.ships[1][7]) end
  ...
  },
  {
    ...
    [[
    local sky=require('stardust.sky')
    local cfg=require('stardust.config').resolve({stars=5,meteors=false,objects={}})
    local colors=require('stardust.palette').setup(cfg)
    local layout=require('stardust.layout')
